...
const string strMessageMagic = "RYCN Signed Message:\n";
...
int64_t GetProofOfWorkReward(int64_t nFees)
{
   ...
   if (pindexBest->nHeight <= 1)
      {
        int64_t nSubsidy = 1250000 * COIN;
        return nSubsidy + nFees;
      }
      
    else if (pindexBest->nHeight <= 110)
      {
        int64_t nSubsidy = 1 * COIN;
        return nSubsidy + nFees;
      }
      
    else if (pindexBest->nHeight <= 144000)
      {
        int64_t nSubsidy = 0 * COIN;
        return nSubsidy + nFees;
      }      
...
bool CheckDiskSpace(uint64_t nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < nMinDiskSpace + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        uiInterface.ThreadSafeMessageBox(strMessage, "RYCN", CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        StartShutdown();
        return false;
    }
    return true;
